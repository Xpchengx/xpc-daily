...
```
index.html      全部界面 + 逻辑（约 3721 行，全内联 —— 原因见下方约束）
...
2. **卡片折叠编号 `data-fold="cN"` 是写死的，新卡片只能用新编号，绝不许重排。** 用户的折叠状态按编号存在 Store 里，重排会让状态串到别的卡片。目前用到 c34（c34 = 番茄钟页的卡片）。⚠️ 已经空出来的编号（c10~c13，原属已删除的年度/日志页）**不要回收复用**，否则用户残留的折叠状态会串到新卡片上。
...
| `Report` | **日报**：自动汇总某一天的任务/完成/备注（`data(date)`/`md(date)` 纯函数 + `render()`），复用 Todo/Plan/Pomodoro；按日期查看（翻页/选日期）+ 导出/复制 Markdown |
| `Pomodoro` | **番茄钟**：专注/短休/长休三段循环（默认 25/5/15，每 4 个长休，可调）。独立页面（侧边栏「🍅 番茄」）。墙钟计时（`endAt`）防走偏；自调度 `setTimeout`（**不用 setInterval**）；运行时状态存本地 `_pomo`、完成记录按天存 `pomo:<日期>`；完成时震动 + WebAudio 提示音；供日报汇总 |
| `Docs` | 云文档入口（链接墙）|

> **已移除**：`Goal`（年度目标）、`Log`（工作日志）两个模块及页面于 2026-09-23 按用户要求整体删除；连带去掉了设置页的「工作日志同步」开关、日报里的「当日日志」卡片、Obsidian 导出里的「工作日志」段。存储里的旧 `goal` / `log` 记录**未主动清除**（仍会进备份文件），需要时可恢复。
...
5. ~~**工作日志默认不上云**（设置页有开关）~~ —— 2026-09-23「工作日志」功能整体移除，本条作废。`Store.NO_SYNC` 仍保留 `['log']` 默认值，是为了让用户残留的旧日志**不上云**（删掉反而会把旧日志推上云端），**别改成 `[]`**。
...
12. **番茄钟（2026-09-23 新增）现在是独立页面 `page-pomo` + 侧边栏「🍅 番茄」入口。** 初版曾放在「今天」页卡片；同日用户要求「把番茄钟放旁边」，遂移到侧边栏（并删掉「年度」「日志」两个入口，净入口数 8，仍 ≤9）。**计时不用 `setInterval`**：用自调度 `setTimeout`（每 250ms 一拍），并用「墙钟时间戳 `endAt` − now」算剩余——息屏/切后台/刷新都不走偏。运行时状态存 `_pomo`（下划线开头 → 不参与云同步，见 `mergeRemote` 跳过 `_` 键），完成的专注记录按天存 `pomo:<日期>`（数组，可同步，供日报汇总）。**只有「专注」阶段计入统计，短休/长休和"跳过"都不计**。阶段结束后**不自动续跑**（避免人不在时自动刷出假番茄），只震动+响一声提醒。回前台时 `Pomodoro.resume()` 续上计时。折叠卡片沿用 c34（编号不变 → 用户既有的收起状态继续有效）。改这里别把 `setInterval` 加回来、别让休息也计入统计。
13. **「年度目标」「工作日志」于 2026-09-23 按用户要求整体删除。** 删掉的是：两个 nav 入口 + 两个 `<section class="page">` + `Goal`/`Log` 两个 JS 模块 + 设置页「日志同步」卡片与相关方法 + 日报页「当日日志」卡片及其 `data/md/render` 分支 + Obsidian 导出里的「工作日志」段 + `.bar`/`.stat` 死 CSS。**删除只删功能、不清数据**：`Store('goal')` / `Store('log')` 里的旧记录仍在（旧日志默认不上云，见决策 5），需要时可恢复。若以后再删功能，照这个清单逐处清，别漏了日报/Obsidian 这类"下游消费者"。
...
# 仅撤销「删年度/日志 + 番茄钟独立页」这次改动：git push -f origin c7f4c52:main
